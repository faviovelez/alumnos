class MaestrosController < ApplicationController
  ...
  def vincular_con_salon_post
    @maestro = Maestro.find(params[:maestro_id])
    @salon = Salon.find(params[:salon])
    @maestro.salons << @salon
    redirect_to vincular_con_salon_path(@maestro.id), alert: "Se asignó el #{@salon.nombre} al maestro #{@maestro.nombre}."
  end
  ...
  private
    ...
    def maestro_params
      params.require(:maestro).permit(:nombre, :edad, :direccion, :salon_id)
    end
end
